////////// DOM ARRAYS ////////////////////////////
  ...
  function insertHTML(item,frag,node,owner,myid){
    const t = document.createComment('');
    frag.parent.insertBefore(t,node);
    const newid=Symbol('$item');
    owner[myid][newid]={frag:true}; //required for _extractunmounts &&  _staleChildren
    const _frag = renderClient(t,item,owner[myid][newid]);
    if(_frag.prev) _frag.prev.nextElementSibling.myid=newid;
    return t;
  }

  //replace node with item
  function replaceHTML(item,node,owner,myid){
    unmountSingleNode(node.myid,owner,myid);
    const newid=Symbol('$item');
    owner[myid][newid]={frag:true}; //required for _extractunmounts &&  _staleChildren
    const _frag = renderClient(node,item,owner[myid][newid]);
    if(_frag.prev)  _frag.prev.nextElementSibling.myid=newid;
  ...
  function unmountSingleNode(nodeid,owner,myid){
    const unmountlist = _extractunmounts(owner[myid][nodeid]);
    if(unmountlist.length) unmountlist.forEach(f=>(typeof f==='function'&&f()));
    _staleChildren(owner[myid][nodeid])
    delete owner[myid][nodeid]
  }

  function unmountAllNodes(owner,myid){
    const unmountlist = _extractunmounts(owner[myid]);
    if(unmountlist.length) unmountlist.forEach(f=>(typeof f==='function'&&f()));
    _staleChildren(owner[myid])
    Object.getOwnPropertySymbols(owner[myid]).forEach(k=>delete owner[myid][k]);
  }
  ...
  function diffArrays(frag, a=[], b=[], fn, owner, myid) { //a = old, b=new
    let stime = DEBUGarr&&Date.now();
    refreshFragment(frag);
    DEBUGarr && console.log('diffArrays',owner[myid],frag,a,b);
      ...
      if(aEnd===aStart) {
        ...
        while (bStart < bEnd) {
          if(temp && temp[bStart]) {
            ...
            temp[bStart]= insertHTML(()=>fn(b[bStart]), frag, node, owner, myid);
          }
          bStart++;
        }
      }
      // fast path: remove head or tail
        ...
        while (aStart < aEnd) {
          ...
          if (!map || !map.has(a[aStart])) {
            ...
            unmountSingleNode(mapped[aStart].myid,owner,myid);
      ...
      // map based fallback, "slow" path
        ...
        if(!tempidx) {
          ...
          if(!reusingNodes) {
            ...
            unmountAllNodes(owner,myid);
            clearFragment(frag);
            return diffArrays(frag,[],b,fn,owner,myid);
          }
        }
          ...
          if (bStart < index && index < bEnd) {
            ...
            if (sequence > (index - bStart)) {
              ...
              if(temp && temp[bStart]) {
                ...
                temp[bStart]= replaceHTML(()=>fn(b[bStart]), mapped[aStart], owner, myid);
              }
              aStart++;
              bStart++;
            }
