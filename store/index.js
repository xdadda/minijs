...
export default function store(...args) {
  ...
  let tstore;
  //upon first call store will be initialized with SERVER/CLIENT session context data
  if(import.meta.env.SSR) { //SERVER
    if(!serverState('zstore')) {
      const session=serverState().session;
      tstore = createStore((set) => ({...initstate,...session}));
      serverState('zstore',tstore);
    }
    else tstore=serverState('zstore'); 
  } 
  else { //CLIENT
    if(!zstore) {
